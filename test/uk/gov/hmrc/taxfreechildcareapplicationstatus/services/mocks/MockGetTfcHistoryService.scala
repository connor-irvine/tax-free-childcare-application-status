...
package uk.gov.hmrc.taxfreechildcareapplicationstatus.services.mocks
...
trait MockGetTfcHistoryService extends MockitoSugar with BeforeAndAfterEach {
  this: Suite =>

  override def beforeEach(): Unit = {
    super.beforeEach()
    reset(mockGetTfcHistoryService)
  }

  val mockGetTfcHistoryService: GetTfcHistoryService = mock[GetTfcHistoryService]

  def stubGetClaimsHistory(nino: String,
                           uniqueClaimId: String
                          )(response: Future[GetTfcHistoryResponse]): Unit = {
    when(mockGetTfcHistoryService.getClaimsHistory
