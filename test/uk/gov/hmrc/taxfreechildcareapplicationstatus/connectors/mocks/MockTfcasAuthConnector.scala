...
trait MockTfcasAuthConnector extends MockitoSugar with BeforeAndAfterEach {
  this: Suite =>

  val mockAuthConnector: TfcasAuthConnector = mock[TfcasAuthConnector]
  ...
  def mockAuth(): Unit =
    when(mockAuthConnector.authorise[Unit](
      ArgumentMatchers.eq(AuthProviders(PrivilegedApplication)), ArgumentMatchers.any()
